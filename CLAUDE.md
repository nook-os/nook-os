...
- **A node with no room left stops claiming, and says so (MAIN-618).** Each heartbeat now carries free/total bytes for the two filesystems a loop job fills — the node's job cache and Docker's data root, reported ONCE when they are the same one — and the node compares them against `NOOK_MIN_FREE_DISK_GB` (default 20). Below it, jobs stay **`queued` under `QueuedReason::DiskUnavailable`**, never `failed`: the shortage is the machine's and failing would spend the card's strike budget on it, exactly `SandboxUnavailable`'s argument. The gate is kind-blind — a spec run writes a checkout too — and recovery is one healthy sample: nothing to restart, nothing to clear. `nook get nodes` has a `disk` column reading `12G LOW`, so *"is azul out of space"* has an answer without a shell on azul.
  - **The floor is stated on the MACHINE only** — env var, no central write and no host pin, unlike loop capacity above. It tracks a physical property of the box, so the box says it; the control plane never learns the number and only reads the sentence the node composed.
  - **Silence is UNKNOWN here, not "full"** — the opposite of the sandbox gate's reading of it. An agent predating the field never reports disks, and gating on that would cordon a whole fleet the day the control plane was upgraded. The sandbox may fail closed because there silence IS the hazard; a full disk merely fails the run that fills it.
