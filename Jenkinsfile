...
pipeline {
    ...
    stages {
        stage('Rust') {
            steps {
                script {
                    // Same build dependencies as control.Dockerfile — curl is
                    // load-bearing: utoipa-swagger-ui downloads the UI bundle
                    // from its build script, and fails the build without it.
                    inImage(RUST_IMAGE, '',
                        'set -e; ' +
