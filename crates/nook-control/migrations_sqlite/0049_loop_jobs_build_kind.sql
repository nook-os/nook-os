-- SQLite twin of 0049 (hand-authored, per CLAUDE.md).
--
...
-- account). So, as in 0040: rewrite the DECLARED schema only. The statement
-- below is the CURRENT stored text — 0040's rewrite plus the 0046/0047
-- ADD COLUMN splices, dumped from a database built by 0001..0048 — with
-- exactly one edit: the kind CHECK gains 'build'.
...
SET sql = 'CREATE TABLE loop_jobs (
  ...
  PRIMARY KEY (id),
  CHECK ((kind IN (''spec'', ''decompose'', ''review'', ''build''))),
