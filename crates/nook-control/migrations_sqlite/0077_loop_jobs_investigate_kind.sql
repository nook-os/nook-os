-- SQLite twin of 0077 (hand-authored, per CLAUDE.md).
--
...
-- account). So, as in 0040, 0049 and 0050: rewrite the DECLARED schema only.
-- The statement below is the CURRENT stored text — dumped from a database built
-- by 0001..0076 — with exactly one edit: the kind CHECK gains 'investigate'.
...
SET sql = 'CREATE TABLE loop_jobs (
  ...
  queued_reason TEXT,
  seed TEXT, review_pr_number bigint, review_head_sha text, review_verdict text, build_outcome text, build_fingerprint text, review_forced INTEGER NOT NULL DEFAULT 0, queued_reason_kind TEXT, review_verdict_source TEXT,
  PRIMARY KEY (id),
...
CREATE TABLE loop_jobs_schema_bump_329 (x);
DROP TABLE loop_jobs_schema_bump_329;
