...
//!
//! `AUTO_JOIN_DEFAULT_TENANT=true` opts out, putting every new identity into
//! the oldest tenant instead. That is the single-team instance — everyone
//! shares one workspace deliberately — and it is off by default because
//! getting it wrong silently exposes one person's work to another.
//!
...
pub async fn login_identity(state: &AppState, claims: IdentityClaims) -> ApiResult<(User, Tenant)> {
    ...
    let (tenant, role) = if identity_count == 0 {
        ...
        // Single-team instance: everyone lands in the oldest tenant and shares
        // its nodes and workspaces. Opt-in, because it is indistinguishable
        // from a leak when it wasn't intended.
        let tenant: Tenant = sqlx::query_as("SELECT * FROM tenants ORDER BY created_at LIMIT 1")
            .fetch_one(&state.db)
            .await?;
        (tenant, "member")
    } else {
        // The default: their own tenant, which they own.
        (
