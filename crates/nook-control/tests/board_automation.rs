...
async fn a_user(db: &PgPool, tenant: TenantId) -> UserId {
    let user = UserId::new();
    sqlx::query(
...
async fn move_fires_label_actions_and_same_column_is_silent() {
    ...
    .await;
    let task = task_in(&bed.pool, &f, f.todo, 1).await;
...
async fn drag_patch_path_also_fires() {
    ...
    .await;
    let task = task_in(&bed.pool, &f, f.todo, 1).await;
    let user = a_user(&bed.pool, f.tenant).await;
...
async fn a_failed_action_records_event_and_comment_but_the_move_sticks() {
    ...
    // The move stuck.
    assert_eq!(column_of(&bed.pool, task).await, f.done);
...
async fn notify_action_raises_a_notification_with_a_deep_link() {
    ...
    .await;
    let task = task_in(&bed.pool, &f, f.todo, 7).await;
