...
/// MAIN-331: an `investigate` run is in the dispatch order like any other kind.
///
/// The inverse of what MAIN-329 asserted, and the same fact from the other
/// side. That exclusion existed because no node could advertise the kind, so
/// the rows would have sat at the head of the order forever — `DISPATCH_PASS_LIMIT`
/// documents that such a head is never lifted by the window itself. A node
/// advertising `investigate` makes the wait an ordinary one, so the row takes
/// its turn by priority and age with everything else.
///
/// The runs here are older than the spec job and on unset-priority cards, so a
/// build that still excluded them would hand the executor to the spec job and
/// fail the first assertion.
#[tokio::test]
async fn investigate_runs_take_their_turn_in_the_dispatch_order() {
    let Some(mut bed) = TestBed::new().await else {
        return;
    };
    ...
    let (user, person) = bed.user(tenant, "owner").await;
    let b = board(&bed, tenant).await;
    let _node = node(&bed, tenant, person, 1, &["spec", "investigate"]).await;
    let state = bed.app_state().await;

    let investigating = queued_at(
        &bed,
        tenant,
        user,
        card(&bed, tenant, b, user, 0).await,
    ...
    assert_eq!(
        order,
        vec![investigating, newer],
        "the investigate run is in the order, and its two hours of waiting win"
    );
    assert_eq!(
        placed_one(&state, tenant).await,
        Some(investigating),
        "and the freed executor goes to it"
    );
