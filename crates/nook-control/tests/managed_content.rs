...
async fn seed_rules_insert_noop_and_refresh() {
    ...
    managed::upsert_default(&bed.pool, "skill", &name, "DEFAULT-V1")
        .await
        .unwrap();
    ...
    managed::upsert_default(&bed.pool, "skill", &name, "DEFAULT-V2")
        .await
        .unwrap();
...
async fn seed_populates_the_real_managed_set() {
    ...
    managed::seed(&bed.pool).await.unwrap();
    ...
    let payloads = managed::managed_skills_as_install(&bed.pool).await.unwrap();
