...
async fn renews_after_expiry_and_across_a_rotation() {
    ...
    ca::promote(&cas(&bed), tenant, new_ca.id).await.unwrap();
    ...
    let renewed = ca::sign_node_csr(&cas(&bed), &v, tenant, node, &csr2)
        .await
        .unwrap();
    assert_eq!(
...
async fn a_valid_certificate_identifies_its_node() {
    ...
    let id = ca::verify_node_cert(&cas(&bed), &nodes(&bed), &der_of(&leaf.cert_pem))
        .await
        .unwrap();
...
async fn a_certificate_from_another_tenant_is_rejected() {
    ...
    let err = ca::verify_node_cert(&cas(&bed), &nodes(&bed), &der_of(&forged.cert_pem))
        .await
        .unwrap_err();
    assert!(
...
async fn a_self_signed_certificate_is_rejected() {
    ...
    let err = ca::verify_node_cert(&cas(&bed), &nodes(&bed), forged.der())
        .await
        .unwrap_err();
    assert!(
...
async fn a_revoked_node_is_refused() {
    ...
    let err = ca::verify_node_cert(&cas(&bed), &nodes(&bed), &der_of(&leaf.cert_pem))
        .await
        .unwrap_err();
