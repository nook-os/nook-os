...
fn tenant_admin_never_gets_ca_rotation() {
    let migrations = fs::read_to_string(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/migrations/0015_rbac_scope.sql"
    ))
    .expect("0015 must be readable");
...
fn operator_holds_every_permission_its_own_surface_requires() {
    ...
    for entry in fs::read_dir(dir).expect("migrations dir") {
        ...
        if path.extension().is_some_and(|e| e == "sql") {
            granted.push_str(&fs::read_to_string(&path).unwrap_or_default());
        }
    }
