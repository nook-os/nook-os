...
async fn set_ledger(pool: &PgPool, rows: &[(i64, Vec<u8>)]) {
...
async fn ledger(pool: &PgPool) -> Vec<(i64, Vec<u8>)> {
...
async fn collapses_a_matching_pre_squash_ledger_to_the_single_row() {
    ...
    let m = manifest(28);
    set_ledger(&bed.pool, &m.old).await;

    let out = restamp(&bed.pool, Some(&m), "_sqlx_migrations")
        .await
        .expect("re-stamp");
    ...
    assert_eq!(ledger(&bed.pool).await, vec![(1, m.new_checksum.clone())]);
    let desc: String = sqlx::query_scalar("SELECT description FROM _sqlx_migrations")
        .fetch_one(&bed.pool)
        .await
        .unwrap();
    assert_eq!(desc, "init");
...
async fn a_second_boot_is_a_no_op() {
    ...
    restamp(&bed.pool, Some(&m), "_sqlx_migrations")
        .await
        .expect("first");
    let after_first = ledger(&bed.pool).await;

    let out = restamp(&bed.pool, Some(&m), "_sqlx_migrations")
        .await
        .expect("second");
    assert_eq!(out, Restamp::AlreadySquashed);
    assert_eq!(ledger(&bed.pool).await, after_first, "untouched");
...
async fn an_unrecognised_ledger_is_refused_and_left_untouched() {
    ...
    set_ledger(&bed.pool, &with_extra).await;
    let err = restamp(&bed.pool, Some(&m), "_sqlx_migrations")
        .await
    ...
    assert_eq!(ledger(&bed.pool).await, edited, "untouched");
...
async fn a_virgin_database_and_a_build_with_no_squash_both_pass_through() {
    ...
    assert_eq!(
        restamp(&bed.pool, None, "_sqlx_migrations").await.unwrap(),
        Restamp::NoManifest
    );

    // Empty ledger: the migrator will apply the canonical 0001 itself.
    set_ledger(&bed.pool, &[]).await;
    assert_eq!(
        restamp(&bed.pool, Some(&manifest(28)), "_sqlx_migrations")
            .await
            .unwrap(),
        Restamp::EmptyLedger
    );
    ...
    assert_eq!(
        restamp(&bed.pool, Some(&manifest(28)), "_no_such_ledger")
            .await
            .unwrap(),
        Restamp::EmptyLedger
    );
...
old 2 2222
";
    let before = ledger(&bed.pool).await;
    ...
    nook_db::migrate::run_boot_migrations(&nook_control::MIGRATOR, &bed.pool, false, bogus)
        .await
