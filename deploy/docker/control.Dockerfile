...
RUN --mount=type=cache,id=nook-cargo-registry,target=/usr/local/cargo/registry,sharing=locked \
    ...
    cargo build --release -p nook-control -p nook-node \
    && mkdir -p /out /dist \
    && cp target/release/nook-control /out/ \
...
COPY --from=build /out/nook-control /usr/local/bin/nook-control
