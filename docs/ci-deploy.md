...
until you put one there. Compose behind Traefik has one, but **ships no wildcard
router** and sets no `TUNNEL_DOMAIN` — the proxy exists and the rule is still
yours to add. The Helm chart writes both: set `ingress.tunnelHost` and it renders
a `*.<zone>` ingress rule pointed at the control-plane Service and sets
`TUNNEL_DOMAIN` from that same value, leaving you the wildcard DNS record and the
certificate.
