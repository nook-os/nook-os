...
**One deployment mode writes that rule for you: the Helm chart.** Set
`ingress.tunnelHost` and it emits the wildcard ingress rule *and* the matching
`TUNNEL_DOMAIN` (MAIN-512, `charts/nook-control/README.md`); the certificate and
the DNS record are still yours. Everywhere else the rule is hand-written —
Compose behind Traefik has a proxy the contract can be satisfied on but emits no
wildcard router and sets no `TUNNEL_DOMAIN`, and the remaining modes have no
proxy at all. So this page is for every deployment, not for "everyone else": on
Traefik you are adding a rule to a proxy that already exists, and on the rest you
are standing one up first.
...
front of the control plane at all, one has a proxy whose configuration stops at
the apex, and the Helm chart has the rule but only once you name the zone.
...
examples below are for; for Traefik it means one more router on the proxy you
already run; for the chart it means one value.
