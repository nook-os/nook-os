// Invite people into your tenant (MAIN-6).
//
// An owner/admin creates a pending invite for an email + role; the server
// returns an opaque accept link, shown here once with a copy button (emailing
// it is MAIN-7). The invitee opens the link, signs in as that email, and is
// added to the tenant. Members can't manage invites, so the whole panel is
...
export function Invites() {
  ...
  const create = async () => {
    ...
    if (data.accept_url) {
      await notify(
        `Invite ready for ${data.email}`,
        "Send them this link. They sign in as this email to join your tenant:",
