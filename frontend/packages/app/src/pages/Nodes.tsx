...
export function NodesPage() {
  const [joinCmd, setJoinCmd] = useState<string | null>(null);
  ...
  const addNode = async () => {
    ...
    if (data) {
      setJoinCmd(
        `nook join --server ${window.location.origin} --token ${data.token}`,
      );
    }
  };
  ...
  return (
    <div className="nook-grid" style={{ gridTemplateColumns: "1fr" }}>
      <Panel
        title="Nodes"
        actions={
          <button className="btn primary small" onClick={addNode}>
            + add node
          </button>
        }
      >
        {joinCmd && (
          <div
            style={{
              padding: 10,
              borderBottom: "1px solid var(--nook-border)",
              background: "var(--nook-bg-panel)",
            }}
          >
            <div className="muted small">
              Run this on the machine you want to add (token valid 24h, shown
              once):
            </div>
            <div className="mono bright" style={{ userSelect: "all", padding: "6px 0" }}>
              {joinCmd}
            </div>
            <button
              className="btn small"
              onClick={() => navigator.clipboard.writeText(joinCmd)}
            >
              copy
            </button>{" "}
            <button className="btn small" onClick={() => setJoinCmd(null)}>
              dismiss
            </button>
          </div>
        )}
