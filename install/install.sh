...
usage: install.sh [options]
  ...
  --k8s                  Kubernetes: install the binary, then run `nook k8s init`
...
ask_role() {
  ...
  fi
  {
    ...
    printf '    [3] %sKubernetes%s — generate Helm values + the install commands\n' "$B" "$R"
    printf '  Choice [1]: '
  } > /dev/tty
...
#
# Kubernetes: the binary generates the values file and prints the install
# commands (`nook k8s init`). With a terminal, launch it now. Under `curl … | sh`
# with no terminal there is nothing to prompt on, so print the one next step and
# stop — the operator runs it when ready (AC-6). The tty probe runs in a subshell
# because a failed /dev/tty redirect is fatal in dash.
if [ "$ROLE" = "k8s" ]; then
  if (exec < /dev/tty) 2>/dev/null; then
    # shellcheck disable=SC2086
    exec "$PREFIX/nook" k8s init $PASSTHRU
  fi
  echo
  say "Next: generate your Helm values and the install commands with"
  say "    nook k8s init"
  exit 0
fi
