...
while IFS= read -r t; do
  [ -n "$t" ] || continue
  printf '%s\n' "$excluded" | grep -Fxq "$t" || regressions="${regressions}${t}"$'\n'
done < <(printf '%s\n' "$FAILED")
...
# (2) An excluded test passed. Without this the list never shrinks.
stale=""
while IFS= read -r t; do
  [ -n "$t" ] || continue
  if printf '%s\n' "$PASSED" | grep -Fxq "$t"; then
    stale="${stale}${t}  (passes now)"$'\n'
  elif ! printf '%s\n' "$FAILED" | grep -Fxq "$t"; then
    stale="${stale}${t}  (no such test in this run)"$'\n'
  fi
done < <(printf '%s\n' "$excluded")
